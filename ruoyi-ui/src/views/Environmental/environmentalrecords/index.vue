...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getEnvironmentalrecords(recordID).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.recordID != null) {
            updateEnvironmentalrecords(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addEnvironmentalrecords(this.form).then(response => {
